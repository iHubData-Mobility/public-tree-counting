...
if __name__ == '__main__':
	...
	parser.add_argument('--video-path', type=str, help='path of the video')
	parser.add_argument('--gpx-filename', type=str, default='GH017756.gpx', help='filename of the .gpx file')
	parser.add_argument('--segment-duration', type=int, default='120000', help='duration in milliseconds of the segmented video')
	...
	segment_duration = opt.segment_duration * 1000
	...
	time_range = int(segment_duration / interval)

	# Path to the folder which contains the video
	video_path = opt.video_path
	video_path_for_macOS = opt.video_path
	# video_path = '/Volumes/My Passport/treeCountingVideos/trip4/Main Video/'
	# This is only needed because certain folder names in my directory had spaces. If in case there are no spaces in your folder names, keep this the same as video_path
	# video_path_for_macOS = '/Volumes/My\ Passport/treeCountingVideos/trip4/Main\ Video/'

	gpx_filename = opt.gpx_filename
	...
	# Step 2: Clip videos into smaller equal time videos. the duration of the smaller video is given by "segment_duration" variable
